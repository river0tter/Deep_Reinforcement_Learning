...
class AgentDQN(Agent):
    def __init__(self, env, args):
        ...
        self.batch_size = 128
        self.num_timesteps = 1500000 # total training steps
    ...
    def make_action(self, state, test=False):
        ...
        # state.shape: [1, 4, 84, 84], action.shape: [1, 9]
        if not test:
            rand = random.random()
            if rand > self.eps:
                with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                action = self.target_net(state.cuda()).max(1)[1].item()
        return action
    ...
    def update(self):
        ...
        batch_trans = Transition(*zip(*transitions))

        batch_state = torch.cat(batch_trans.state).cuda()
        batch_action = torch.cat(batch_trans.action)
        batch_reward = torch.cat(batch_trans.reward)
        non_terminal = torch.tensor(tuple([s is not None for s in batch_trans.next_state]), dtype=torch.bool).cuda() #mask out states with no next states
        next_state = torch.cat([s for s in batch_trans.next_state if s is not None]).cuda()
        
        #step 2
        Q_st_a = self.online_net(batch_state).gather(1, batch_action)
        
        #step 3
        with torch.no_grad():
            V_st_plus1 = torch.zeros(self.batch_size).cuda()
            V_st_plus1[non_terminal] = self.target_net(next_state).max(1)[0].detach()
        
        #step 4
        E_Q_st_a = batch_reward + (self.GAMMA * V_st_plus1) 
        E_Q_st_a = E_Q_st_a.unsqueeze(1)
        
        #step 5
        loss = F.mse_loss(Q_st_a, E_Q_st_a)
    ...
    def train(self):
        ...
        while(True):
            ...
            state = torch.from_numpy(state).permute(2,0,1).unsqueeze(0)
            episode_reward = 0 # for tensorboard plotting
            done = False
            while(not done):
                ...
                action = torch.tensor(action).view(1, 1).cuda()
                # process new state
                next_state = torch.from_numpy(next_state).permute(2,0,1).unsqueeze(0)

                # TODO: store the transition in memory
                # if buffer is full, replace the oldest transition with new gerenated transition. 
                if len(self.buffer) < self.buffer_size:
                    self.buffer.append(None)
                self.buffer[self.pointer] = Transition(state, action, next_state, reward)
                ...
                self.steps += 1
            #writer.add_scalar('episode_reward', episode_reward, episodes_done_num + 1)
            if episodes_done_num % self.display_freq == 0:
                print('Episode: %d | Steps: %d/%d | Avg reward: %f | loss: %f '%
                        (episodes_done_num, self.steps, self.num_timesteps, total_reward / self.display_freq, loss))
            
                #writer.add_scalar('reward', total_reward / self.display_freq, episodes_done_num)
                total_reward = 0
